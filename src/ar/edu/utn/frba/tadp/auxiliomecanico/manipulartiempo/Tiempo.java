...
/**
 * La idea de la clase tiempo es poder manejar horas y minutos de manera
 * independiente, y si el dia de mañana se encuentra una solucion mejor para la
 * manipulacion del tiempo se puede solo cambiar la clase ademas se evita entrar
 * en un "primitive obseccion"
 * 
 * @author Nico
 * 
 */
public class Tiempo {
	...
	public void setHoras(int hora) {
		if ((hora < 0)) {
			...
			throw new HorasIncorrectasException(
					"La cantidad de horas es incorrecta");
		}
		this.horas += hora;
	}
	...
	public void setMinutos(int minuto) {
		if ((minuto < 0)) {
			this.horas = 0;
			this.minutos = 0;
			throw new MinutosIncorrectosException(
					"La cantidad de minutos es incorrecta");
		}
		if (minuto > 60) {
			horas += (minuto / 60);
			minutos = ((minuto % 60));
	...
	public static Tiempo sumarTiempos(Tiempo primerTiempo, Tiempo sumando) {
		...
		// Primero sumamos los minutos teniendo en cuenta el overflow de los
		// mismos
	...
	public static boolean esMayor(Tiempo t1, Tiempo t2) {
		return ((t1.horas >= t2.horas) && (t1.minutos > t2.minutos));
	}
